...
-- Periodically pre-fetch closest vehicle's NOS data for smooth target checks
CreateThread(function()
    while true do
        local ped = cache.ped
        if ped and not IsPedInAnyVehicle(ped, false) then
            ...
            if vehicle and vehicle ~= 0 then
                ...
                if not state then
                    local plate = GetVehicleNumberPlateText(vehicle)
                    local netId = NetworkGetNetworkIdFromEntity(vehicle)
                    if plate and netId and netId ~= 0 then
                        TriggerServerEvent('npds_nos:server:requestNOSDataSync', plate, netId)
                    end
                end
            end
        end
